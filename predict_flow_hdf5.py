...
def predict(input_hdf5, output_hdf5, dataset_path, output_dataset_path, checkpoint_path):
    ...
    num_pairs = N - 1
    flow_array = np.zeros((N, H, W, 2), dtype=np.float32)  # flow_array[0] stays zeros
    ...
    with torch.no_grad():
        for i in range(num_pairs):
            ...
            flow_array[i + 1] = result.cpu().numpy().transpose(1, 2, 0)
    ...
    # Restore NaN: if either frame in a pair had NaN, set output to NaN (skip row 0 which is zeros)
    pair_nan = nan_mask[:num_pairs] | nan_mask[1:]  # (num_pairs, orig_H, orig_W)
    flow_array[1:][pair_nan] = np.nan  # broadcasts over last (channel) axis
    nan_fraction = pair_nan.sum() / pair_nan.size
    print(f'Output NaN fraction: {nan_fraction:.3%}')

    with h5py.File(output_hdf5, 'a') as f:
        f.create_dataset(output_dataset_path, data=flow_array, compression='gzip')
    print(f'Flow saved to {output_hdf5}:{output_dataset_path} — shape {flow_array.shape}, dtype {flow_array.dtype}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dataset_path', type=str, default='/m0/q007_0/diff_bg_flow',
                        help='HDF5 dataset group for the output file')
    ...
    predict(input_hdf5, output_hdf5, dataset_path, output_dataset_path, checkpoint_path)